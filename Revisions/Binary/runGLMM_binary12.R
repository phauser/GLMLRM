...
Rcpp::sourceCpp("~/Project3/Code/GLLMRM.cpp")
load("/work/users/p/h/phauser/Sim/Binary/X.Rdata")
...
stats=function(B0, B, PostB){
  ...
  for(i in 1:p){
    for(j in 1:d){
      out       = ci(mod$B0[i,j,], method="HDI")
      MIW[i,j]  = out$CI_high-out$CI_low
      CP[i,j]   = ifelse(B0[i,j]>out$CI_low && B0[i,j]<out$CI_high, 1, 0)
...
for(g in 1:G){
  ...
  mod = GLMLRM2("SS", x1, Y, d1=d1, d2=d2, r1=r0, r2=r0, q=q0, 
                epsb=8, epsU1=1, epsU2=1, epsV1=.5, epsV2=.5, 
                epsD1=.08, epsD2=.08, epsL=.2, epsC=.05,
                nBurnin=1000, nCollect=3000, thin=1, numThreads=nThreads)
...
# save results
name = paste0("binary_12_", ind)
assign(name, RES)
save(res, RES, file=paste0("/nas/longleaf/home/phauser/Project3/Sim/Revisions/Binary12/", name, ".Rdata"))
