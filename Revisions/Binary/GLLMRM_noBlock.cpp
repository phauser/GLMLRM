...
// [[Rcpp::export]]
List GLMLRM(std::string priorC, const arma::mat &X, const arma::mat &Y, 
           int & r, int & q, double & epsb, 
...
List perform(mat Y, mat X, mat B, cube Bh, mat C, cube Ch, mat lambda, mat b, vec dev0){
  ...
  for(int i=0; i<p; i++){
    for(int j=0; j<d; j++){
      vec Bij = Bh.tube(i,j);
      bias(i,j)=abs(mean(Bij)-B(i,j));
      MSE(i,j)=pow(mean(B(i,j)-mean(Bij)),2);
  ...
  return List::create( Named("MSE_B")=median(MSE.as_row()), 
                       Named("bias_B")=median(bias.as_row()),
                       Named("MSE_C")=median(MSEC.as_row()), 
                       Named("bias_C")=median(biasC.as_row()),
                       Named("DIC")=DIC);
}
