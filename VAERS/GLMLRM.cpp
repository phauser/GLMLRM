...
// [[Rcpp::export]]
List GLMLRM2(const arma::mat &X, const arma::mat &Y, 
           int & d1, int & d2, int & r1, int & r2, int & q,
           double & epsE, double & epsU1, double & epsU2, 
  ...
  for(int iter=0; iter<runs; iter++){
    ...
    MH_b = UpdateEta(proposal, as<NumericMatrix>(wrap(Y)), as<NumericMatrix>(wrap(X)),
        ...
        taub, epsE, numThreads);
    b = MH_b.submat(0, q, b.n_rows-1, 2*q-1);
    AR_E = MH_b.submat(0, 0, b.n_rows-1, q-1);
  ...
  Output["AR_E"]            = mean(SumAR_E/N0);
  Output["AR_lambda"]       = SumAR_lambda/N0;
  return Output;
}

// [[Rcpp::export]]
List GLMLRM1(const arma::mat &X, const arma::mat &Y, 
             int & r, int & q, double & epsE, 
             double & epsU,  double & epsV, double & epsD,  
             double & epsL, double & epsC,
             int & nBurnin, int & nCollect, int & thin, int numThreads){
  ...
  mat B0 = U*diagmat(delta)*V.t();
  
  double tauU = 1.0/sqrt(p);
  double tauV = 1.0/sqrt(d);
  double tauD = sqrt(r);
  ...
  mat AR_lambda     = zeros(d, q);
  mat SumAR_lambda  = zeros(d, q);
  mat MH_U, MH_V, MH_b, MH_lambda, MH_C0, MH_C1, MH_C;
  vec MH_delta;
  ...
  for(int iter=0; iter<runs; iter++){
    ...
    for(int l=0; l<r; l++){
      // Step 1: Update U
      MH_U = UpdateU(l, as<NumericMatrix>(wrap(Y.submat(0,0,n-1,d-1))), as<NumericMatrix>(wrap(X)),
                     as<NumericMatrix>(wrap(b)), as<NumericMatrix>(wrap(lambda.submat(0,0,d-1,q-1))),
                     as<NumericMatrix>(wrap(U)), as<NumericMatrix>(wrap(V)),
                     as<NumericVector>(wrap(delta)), tauU, epsU, numThreads);
      U.col(l)    = MH_U.col(1);
      AR_U.col(l) = MH_U.col(0);
      
      // Step 2: Update V
      MH_V = UpdateV(l, as<NumericMatrix>(wrap(Y.submat(0,0,n-1,d-1))), as<NumericMatrix>(wrap(X)),
                     as<NumericMatrix>(wrap(b)), as<NumericMatrix>(wrap(lambda.submat(0,0,d-1,q-1))),
                     as<NumericMatrix>(wrap(U)), as<NumericMatrix>(wrap(V)),
                     as<NumericVector>(wrap(delta)), tauV, epsV, numThreads);
      V.col(l)   = MH_V.col(1);
      AR_V.col(l) = MH_V.col(0);
      
      // Step 3: Update delta
      MH_delta = UpdateDelta(l, Y.submat(0,0,n-1,d-1), X, b, lambda.submat(0,0,d-1,q-1), U, V, delta, tauD, epsD);
      delta(l) = MH_delta(1);
      AR_delta(l) = MH_delta(0);
    }
    B0 = U*diagmat(delta)*V.t();
...
// [[Rcpp::export]]
List Perform(mat B, cube Bh, mat C, cube Ch){
  int p = B.n_rows;
  int d = B.n_cols;
  ...
  return List::create( Named("MSE(B)")=median(median(MSEB)), 
                       Named("bias(B)")=median(median(biasB)),
                       Named("MSE(C)")=median(median(MSEC)), 
                       Named("bias(C)")=median(median(biasC)));
}
