...
// [[Rcpp::export]]
List GLMLRM_normal2(mat X, mat Y0, int d1, int d2, int r1, int r2, int q, int nBurnin, int nCollect){
  ...
  for(int iter=0; iter<nBurnin+nCollect; ++iter){
    ...
    // Update Lambda - use Cholesky/QR decomp to find the inverse 
    for(int k=0; k<d; ++k){
  ...
  // things for the function to output
  List Output;
  int N0 = nCollect;
  mat PostB = SumB/N0;
  mat PostL = SumL/N0;
...
// [[Rcpp::export]]
List GLMLRM_normal1(mat X, mat Y0, int r, int q, int nBurnin, int nCollect){
  ...
  for(int iter=0; iter<nBurnin+nCollect; ++iter){
    ...
    for(int l=0; l<r; ++l){
      ...
      // sample U, sig_Ul is p x p
      double temp = as_scalar(pow(Delta1(l),2)*V1.col(l).t()*O*V1.col(l)); 
      mat sig_Ul = inv(tau_u1*eye(p,p)+temp*X.t()*X);
      vec mu_Ul = Delta1(l)*sig_Ul*(X.t()*Y*O*V1.col(l));                
      U1.col(l) = rmvnorm(1,mu_Ul,sig_Ul).t();
